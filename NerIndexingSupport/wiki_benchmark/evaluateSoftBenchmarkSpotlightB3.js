/**
 * soft_benchmark
 * nohup node evaluateSoftBenchmarkSpotlightB3.js ./soft_benchmark/ ./soft_benchmark.spotlight.result ./soft_res > evaluateSoftBenchmarkSpotlight.out 2>&1&
 * 
 * 
 * 3d_candidate
 * nohup node evaluateSoftBenchmarkSpotlightB3.js ./3dcandidate_benchmark/ ./3dcandidate.spotlight.result ./3dcandidate_res > evaluate3dCanodidateBenchmarkSpotlight.out 2>&1&
 */
    ...
    callRecService(paragraph, goldenEntity.name, function(entities) {
        ...
        for (var i in entities) {
            ...
            if (entities[i].name == goldenEntity.name) {
                fs.appendFileSync(resultDir + "/spotlight.tab", fileName + "\t" + decodeURI(entities[i].link.replace("http://www.wikipedia.org/wiki/", "")) + "\tNULL\n");
                callback(entities[i].link == goldenEntity.link, entities[i].link);
                return;
            }
        }
        
        fs.appendFileSync(resultDir + "/spotlight.tab", fileName + "\tNIL" + (nilI++) + "\tNULL" + "\n");
        callback();
    });
