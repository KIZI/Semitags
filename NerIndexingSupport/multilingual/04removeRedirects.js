...
var redirectRedisDb = process.argv[3];

if (!lang) {
    console.log("Usage: node 04removeRedirects.js nl redirectRedisDb");
}
...
//            console.log("Found Redirect " + csvRow[0] + " to " + reply);
            csvRow[0] = "http://" + lang + ".wikipedia.org/wiki/" + reply;
...
client.select(redirectRedisDb, function() {
    csv().from(fs.createReadStream(__dirname + '/../../csv/' + lang + 'csv/entities_mentions_cleaned.csv'), {
        delimiter : ',',
        escape: '"',
        relax: true
    })
    .to.stream(fs.createWriteStream(__dirname + '/../../csv/' + lang + 'csv/entities_mentions_no_redirects.csv'))
